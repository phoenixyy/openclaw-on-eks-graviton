...
def init_db():
    ...
    # Create instances table (multi-instance support)
    cursor.execute('''
    ...
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_instances_user_email
        ON instances(user_email)
    ''')
...
def get_next_instance_sequence(user_id: str) -> int:
    """
    Get the next available instance sequence number for a user

    Args:
        user_id: User ID (8-character hash)

    Returns:
        Next available sequence number (0-99)
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    if not row:
        # No instances yet, start at 01
        return 1

    # Extract sequence number from instance_id (format: {user_id}-{seq})
    last_instance_id = row[0]
    try:
        last_sequence = int(last_instance_id.split('-')[-1])
        return last_sequence + 1
    except (ValueError, IndexError):
        # Fallback if parsing fails
        return 1

def create_instance_record(instance_id: str, user_id: str, user_email: str,
                          display_name: str, provider: str, model: Optional[str] = None) -> int:
    """
    Create an instance record in the database

    Args:
        instance_id: Instance ID (format: {user_id}-{seq})
        user_id: User ID
        user_email: User email
        display_name: Human-readable instance name
        provider: LLM provider (bedrock, siliconflow)
    ...
    cursor.execute('''
        INSERT INTO instances (instance_id, user_id, user_email, display_name, provider, model, status)
        VALUES (%s, %s, %s, %s, %s, %s, 'pending')
        RETURNING id
    ''', (instance_id, user_id, user_email, display_name, provider, model))
    ...
    logger.info(f"✅ Created instance record: {instance_id} for user {user_email}")
    return record_id

def get_user_instances(user_email: str) -> list:
    """
...
def delete_instance_record(instance_id: str) -> bool:
    ...
    if deleted:
        logger.info(f"✅ Deleted instance record: {instance_id}")
    return deleted

def update_instance_status(instance_id: str, status: str) -> bool:
    """
    Update instance status

    Args:
        instance_id: Instance ID
        status: New status (pending, running, failed, etc.)
